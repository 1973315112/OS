...
struct free_area_tree free_area;
...
{
    //scprintf("调试信息:进入buddy_system_init_memmap():实际物理页数n=%d \n",n);
    assert(n > 0);
    ...
    //cprintf("调试信息:退出buddy_system_init_memmap()\n");
}
...
static struct Page* buddy_system_alloc_pages(size_t n) 
{
    ...
    //cprintf("[调试信息]退出buddy_system_alloc_pages(),分配的地址page=%x\n",page);
    return page;
}
